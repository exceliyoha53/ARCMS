...
REQUIRED_COLUMNS = {"matric_number", "ca_score", "exam_score"} # column headers case insensitive

def parse_score_sheet(
        file_buffer: BytesIO
    ...
    try:
        workbook = load_workbook(file_buffer, data_only=True) # True here returns cell values not formulas
        sheet = workbook.active # use first sheet

    except Exception as e:
        logger.error(f"failed to open excel file: {e}")

        return [], [{"row": 0, "matric_number": "N/A", "error": f"Could not read file: {e}"}]  # return one top level error
    
    headers = [
        str(cell.value).strip().lower() if cell.value else "" for cell in sheet[1]  # sheet[1] is first row
    ]
    ...
    for row_num, row in enumerate(sheet.iter_rows(min_row=2, values_only=True), start=2):
        ...
        except ValidationError as e:  # pydantic validation failed, extract readable error msg
            error_messages = "; ".join(
