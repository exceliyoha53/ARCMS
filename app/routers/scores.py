...
def upload_scores(
    ...
    try:
        ...
        # parse excel file
        file_bytes = file.file.read()  # read from memory - no disk writes
        file_buffer = io.BytesIO(file_bytes)  # wrap bytes so openpyxl can read it

        parsed_scores, errors = parse_score_sheet(file_buffer)

        uploaded_metrics = [score.matric_number for score in parsed_scores]

        if not uploaded_metrics:
            return {"message": "No valid scores found in the file"}
        ...
        # bulk fetch: get all valid registrations for students
        cursor.execute(
            """
            SELECT s.matric_number, r.id AS registration_id
            FROM students s
            JOIN registrations r ON s.id = r.student_id
            WHERE r.course_offering_id = %s 
            AND s.matric_number = ANY(%s)
        """,
            (course_offering_id, uploaded_metrics),
        )

        # looks like {"U22TE1022": 5, "U22TE1023": 8}
        valid_registrations = {
            row["matric_number"]: row["registration_id"] for row in cursor.fetchall()
        }
        ...
        for score_data in parsed_scores:
            reg_id = valid_registrations.get(score_data.matric_number)

            if not reg_id:  # student not registered in the system - skip with warning
                logger.warning(f"Student {score_data.matric_number} skipped: Not registered for offering {course_offering_id} or does not exist.")
                continue
...
def get_pending_scores(
    ...
    try:
        ...
        logger.info(
            f"HOD {current_user['email']} fetched {len(pending)} pending course uploads"
        )
        return pending
...
def approve_scores(
    course_offering_id: int, current_user: dict = Depends(require_hod)
) -> dict:
    """
    ...
    try:
        ...
        for student_id in affected_students:
            recalculate_student_gpa(student_id, conn)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Approval failed"
        )
