...
def get_student_results(
    matric_number: str, current_user: dict = Depends(get_current_user)
) -> dict:
    """
    ...
    try:  # fetch student record
        ...
        for semester in semester_summaries:
            semester["courses"] = courses_by_semester.get(
        ...
        logger.info(
            f"Results fetched for {matric_number} by {current_user['email']}"
        )
